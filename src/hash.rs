//! Content hashing abstraction with streaming support
//!
//! This module provides a unified interface for computing content hashes
//! using different algorithms. By default, BLAKE3 is used for its exceptional
//! performance on modern hardware. SHA-256 can be enabled via the `sha256` feature.
//!
//! ## Performance
//!
//! - BLAKE3: ~10 GB/s on modern CPUs (single-threaded), highly parallelizable
//! - SHA-256: ~500 MB/s on modern CPUs (single-threaded)
//!
//! BLAKE3 is recommended for all use cases unless SHA-256 compatibility is required.
//!
//! ## Streaming
//!
//! All hash operations use streaming I/O with a 64KB buffer, ensuring constant
...
/// Size of buffer for streaming hash computation (64KB)
///
/// Tuned for modern SSD performance - balances syscall overhead with memory usage.
const HASH_BUFFER_SIZE: usize = 64 * 1024;

/// A content hash that can represent different hash algorithms
///
/// This enum allows the system to work with multiple hash types transparently.
/// The hash bytes are stored in a fixed-size array for efficiency.
...
impl Hasher {
    ...
    /// # Performance
    ///
    /// - Uses buffered I/O with 64KB chunks
